...
# The line below creates a vector that is a sequence of 9 years
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below creates an empty matrix describing the number of columns as the length
# of the vector fish_year0
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below filters to return every column for the first row based on the
# initial conditions and populates it
proj_year[1, ] <- fish_year0

# The for loop below begins with the second row since we just filtered for the 
# first row above. %*% = the dot product. 2 because we already populated our
# first row with initial conditions
